...
| `/test` | `npm run build` 통과 + 수동 동작 체크리스트 확인 |
...
3. **빌드/실행 검증**: `npm run build` 또는 `build-and-run.bat`. test/lint 스크립트는
   없으므로 최소 build 통과 + 수동 확인.
4. **selector 작업 주의**: AI 사이트 DOM 자동화는 `electron/selectors.json`에 집중되어
   있다. selector를 코드에 하드코딩하지 말 것.
5. **비밀정보 금지**: API 키·Telegram 토큰을 코드/로그/git에 넣지 않는다.
   `electron-store` 사용.
6. **Later Phase 기능은 명시 요청 없이 구현하지 않는다** (PROJECT_SCOPE 참고).
