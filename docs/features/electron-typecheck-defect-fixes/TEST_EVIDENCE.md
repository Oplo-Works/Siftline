...
- Overall Result: **BLOCKED / AC-4 FAIL** — automated/code/build/Saved Session/Chat checks pass. The user-operated Kimi logout transition correctly produced `true → false`, but a successful fresh login remained `false`: current Kimi persisted `access_token`/`refresh_token`/user-id storage keys and did not recreate `kimi-auth`. The approved cookie-only predicate therefore misreports the fresh authenticated session and requires a revised SPEC/PLAN before code changes.
...
- Kimi true → logout false → user login true: **FAIL**. Initial persisted state was `true` with exact `kimi-auth`; the user's Logout produced `false` and removed that cookie. The user then completed a fresh login. The Kimi page had non-empty `access_token`, `refresh_token`, and user-id storage entries and no login control, but no `kimi-auth`; after an app restart the IPC result still reported `kimi:false`. This is an authenticated-state predicate mismatch, not a missing user action. S2/bundle completion is not claimed.
...
| AC-4 | FAIL | Actual `true → false` Logout passed, but user-completed Login remained `false` because fresh Kimi auth used storage tokens and did not recreate `kimi-auth`. |
| AC-5 | FAIL | Cookie fixtures still pass and the other six providers remain stable, but the exact-cookie rule rejects the newly observed fresh authenticated state. The acceptance rule needs revision. |
...
- Required blocker: AC-4's actual user-operated cycle failed at `false → true`. Fresh Kimi authentication now presents non-secret renderer storage signals without recreating `kimi-auth`, so S2 and the bundle must not be called complete.
- `isLoginComplete()` Kimi and persisted Accounts status share the approved exact-cookie predicate, but that predicate is invalid for the freshly observed provider state. Changing it requires a revised SPEC/PLAN that defines a safe main/renderer boundary and logout semantics; no speculative code change was made.
